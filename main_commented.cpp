...
//Reading Parameters from the text file
    ...
    if(f.is_open()) //Getting data from the text file
    {
...
{   //setting the values of position, velocities wall particle position etc.
    ...
    for (int i=0; i < x.size(); i++) //updating the values 
    {
...
{   // saving the checkpoint to a csv file
    int numpts = x.size();
    ...
    for (int i=0; i < rw.size(); i++) // putting values in the csv file
    {
...
int main(int argc, char *argv[])
{
  ...
  //Setting arguments
    ...
    saveCheckpoint("final", output_dir, buf_x, buf_v, buf_vw, buf_r, buf_rw, buf_p, buf_pw, x, v, r, p, Q, Nw);// saving the data
}
