...
float kernel_cubic(vector<float> xi, vector<float> xj, float h)
{
    float pi = 3.14159;
...
vector<float> kernel_derivative(vector<float> xi, vector<float> xj, float h)
{   
    float pi = 3.14159;
    float q = distance(xi, xj) / h;
    float dwdq = 0;
 ...
 void DEN(int iterator, vector<vector<float> > &x, vector<vector<float> > &xw, vector<vector<float> > &v, vector<vector<float> > &vw, vector<float> &r, vector<float> &rw, float m, float h, float dt, int N, int Nw)
{
    ...
    for (int j=0; j<N; j++)
    {
        vec[0] = v[i][0] - v[j][0];
        vec[1] = v[i][1] - v[j][1];
        dr += 1 / r[j] * dot(vec, kernel_derivative(x[i], x[j], h));
    }
    vector <float> vect(2);
    for (int j=0; j<Nw; j++)
    {
        vect[0] = v[i][0] - vw[j][0];
        vect[1] = v[i][1] - vw[j][1];
        dr += 1 / rw[j] * dot(vect, kernel_derivative(x[i], xw[j], h));
    }
    r[i] += dr * m * r[i] * dt;
}
 ...
 void UPDATE_VEL(int iterator, vector<vector<float> > &x, vector<vector<float> > &xw, vector<float> &p, vector<float> &pw, vector<vector<float> > &v, vector<vector<float> > &vw,vector<float> &r, vector<float> &rw, float m, int N, int Nw, float dt, float h)
{
    ...
    for (int j=0; j<N; j++)
    {
        if (distance(x[i], x[j]) < 2*h)
        {
            ...
            vector<float> dW = kernel_derivative(x[i], x[j], h);
    ...
    for (int j=0; j<Nw; j++)
    {
        if (distance(x[i], xw[j]) < 2*h)
        {
            ...
            vector<float> dW = kernel_derivative(x[i], xw[j], h);
...
{   
    ...
    for (int j=0; j<N ; j++)
    {
        num_v_w[0] += v[j][0]*kernel_cubic(xw[i], x[j], h);
        num_v_w[1] += v[j][1]*kernel_cubic(xw[i], x[j], h);
...
int main()
{
	double start = clock();
    int N=1024;
    string input_params, output_dir = "Yes";
    int error;
